...
export function defaultCanCreateInviteForRole(args: {
  ...
  const actorIndex = ROLE_HIERARCHY.indexOf(actor as (typeof ROLE_HIERARCHY)[number]);
  const targetIndex = ROLE_HIERARCHY.indexOf(target as (typeof ROLE_HIERARCHY)[number]);
